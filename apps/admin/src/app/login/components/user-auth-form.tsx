...
export function UserAuthForm({ className, ...props }: UserAuthFormProps) {
  ...
  return (
    <div className={cn('grid gap-6', className)} {...props}>
      {fetchedOTP ? (
        <VerifyComponents isLoading={isLoading} setIsLoading={setIsLoading} />
      ) : (
        <TryComponents
          isLoading={isLoading}
...
function TryComponents({ isLoading, setIsLoading, setFetchedOTP }) {
  ...
  const [submitErrors, setSubmitErrors] = React.useState<{ email: string }>({
    email: '',
  })
  ...
  async function onSubmitEmail() {
    try {
      ...
      if (!response.ok) {
        ...
        setSubmitErrors({ email: res.message })
      }
  ...
  return (
    <>
      <div className="grid gap-1">
        ...
        />
        {submitErrors.email && (
          <p className="mt-2 text-sm text-red-700">{submitErrors.email}</p>
        )}
      </div>
      <Button
